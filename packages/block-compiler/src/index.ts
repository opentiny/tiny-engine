...
// @ts-ignore
const compileBlockScript = (descriptor: SFCDescriptor, id: string, resultMap: IResultMap): [string, BindingMetadata | undefined] => {
  // const scriptCode = descriptor.scriptSetup?.content || descriptor.script?.content || ''
  // const resolvedCode = resolveRelativeImport(scriptCode, resultMap)
...
const compileFile = (file: IParsedFileItem, resultMap: IResultMap): Omit<compiledItem, 'blobURL'> => {
  ...
  // console.log('componentCode', compiledScript)
...
const filterNextCompileFiles = (files: IParsedFileItem[], compiledFilesSet: Set<string>) => {
  return files.filter((fileItem) => {
    // 未被编译
    return !compiledFilesSet.has(fileItem.fileNameWithRelativePath)
      // 且子依赖已经被编译
...
export const compile = (fileList: IFileList, config) => {
  ...
  while (nextCompileFile.length) {
    // console.log('nextCompileFile', nextCompileFile)
    // console.log()
    for (const fileItem of nextCompileFile) {
      ...
      // console.log('resolvedImportJs', resolvedImportJs)
