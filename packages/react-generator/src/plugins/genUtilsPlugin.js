...
function genUtilsPlugin(options = {}) {
  ...
  const handleNpmUtils = (utilsConfig) => {
    ...
    const { exportName, destructuring, subName } = content

    // 自动将 @opentiny/vue 替换为 @opentiny/react
    const packageName = content.package.replace('@opentiny/vue', '@opentiny/react')
