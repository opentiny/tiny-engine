...
const translateHookState = (state) => {
  ...
  for (const item in state) {
    const stateValue = state[item]

    // 如果是带有访问器的复杂状态，只取默认值
    if (stateValue && typeof stateValue === 'object' && stateValue.defaultValue !== undefined) {
      map.set(item, stateValue.defaultValue)
    } else {
      map.set(item, stateValue)
    }
  }
