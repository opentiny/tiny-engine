...
├── .env.example          # 环境变量模板
├── server/               # 后端服务核心
│   ├── index.js          # 服务入口（启动HTTP服务、注册路由）
│   ├── controllers/      # 控制器（处理业务逻辑）
│   │   └── materialController.js  # 物料导入核心控制器
│   ├── middlewares/      # 中间件（参数校验、错误处理）
│   │   ├── errorHandler.js       # 全局错误处理
│   │   └── paramsValidator.js    # 请求参数校验
│   ├── routes/           # 路由配置
│   │   └── material.js   # 物料相关接口路由
│   └── utils/            # 工具函数
│       └── taskManager.js # 任务管理（创建、更新、查询）
└── src/                  # 核心业务逻辑
    ├── api-generation/   # API生成模块
    │   ├── file-based-api-generator.js # 文件上传→API生成
    │   └── web-based-api-generator.js  # URL爬取→API生成
    ├── file-collection/  # 文件筛选模块
    │   ├── component-code-file-filter.js # Code类型文件筛选
    │   └── component-npm-file-filter.js  # NPM类型文件筛选
    ├── post-processing/  # 物料后处理
    │   ├── post-process-schemas.js # 物料JSON加工
    │   └── multi-component-handlers/ # 多组件处理工具
    └── schema-conversion/# 物料协议转换
        ├── cli.js        # 日志文件保存
        └── convertor.js  # API→TinyEngine物料转换
```


## 🎯 关键模块说明
### 1. 文件筛选模块（`src/file-collection`）
...
### 2. LLM接口调用失败？
- 检查`.env`中`OPENAI_API_KEY`和`OPENAI_BASE_URL`是否正确。
