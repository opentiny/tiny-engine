...
function adaptToPostProcessFormat(materialJsonArray) {
  ...
  return validSchemas.map(schema => ({
...
---
## 组件识别
- 先分析所有输入的.vue和.ts文件，识别组件关系并统计独立组件数量（如Form与FormItem视为两个独立组件）。
- 按识别出的数量，为每个组件单独生成一个符合指定格式的独立JSON文件。
...
## 输出要求
请生成完整的 JSON 数组，确保格式正确、无额外文字。无论识别出 1 个还是多个组件，均统一输出 JSON 数组：
