...
async function generateApiJsonWithLLM(combinedContent) {
  ...
  - **每个事件项必须包含以下字段**：
    ...
    | \`type\`          | string   | 必选。固定值"Function"（事件本质为函数回调）                                  |
  ...
  - **每个方法项必须包含以下字段**：
    ...
    | \`parameters\`   | string   | 必选。方法参数列表：<br>- 有参数时填类型定义（如\`"(id: string, options?: object)"\`）；<br>- 无参数时填""（空字符串） |
    | \`returnType\`   | string   | 必选。方法返回值类型：<br>- 有返回值时填类型（如"void"、"string"、"Promise<boolean>"）；<br>- 无返回值时填"void"；<br>- 无法确定时填"any" |
