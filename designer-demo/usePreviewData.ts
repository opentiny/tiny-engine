...
import { fetchMetaData, fetchAppSchema, fetchBlockSchema, getPageById, getBlockById, fetchPageHistory, fetchPageList } from './http'
  ...
  const updatePreview = async (params: {
    ...
    if (previewType === 'page') {
      ...
      const getRouterFile = (schema) => {
        let { routes, importSet } = getRoutesAndImportSet(schema)
